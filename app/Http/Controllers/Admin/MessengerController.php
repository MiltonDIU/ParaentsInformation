...
class MessengerController extends Controller
{
    ...
    public function createTopic()
    {
        if(Auth::user()->roles[0]->id==1){
            ...
            $users = User::where('if_admin_message','1')->whereHas('roles', function($query) {
                $query->where('id', 1);
            })->get();
        }
        $feedbackCategories = FeedbackCategory::where('is_active','1')->get();
        $unreads = $this->unreadTopics();
        return view('admin.messenger.create', compact('users', 'unreads','feedbackCategories'));
    }
    ...
    public function storeTopic(QaTopicCreateRequest $request)
    {
        ...
        $topic = QaTopic::create([
            ...
            'feedback_category_id' => $request->input('feedback_category_id'),
        ]);
