    ...
    <div class="card">
        ...
        <div class="card-body">
            <div class="table-responsive">
                <table class=" table table-bordered table-striped table-hover datatable datatable-linkCategoryLinks">
                    <thead>
                    <tr>
                        <th width="10">
                        ...
                        <th>
                            {{ trans('cruds.link.fields.is_active') }}
                        </th>
                        <th>
                            &nbsp;
                        </th>
                    ...
                    @foreach($links as $key => $link)
                        <tr data-entry-id="{{ $link->id }}">
                            ...
                            <td>
                                @foreach($link->link_categories as $key => $item)
                                    <span class="badge badge-info">{{ $item->title }}</span>
                                @endforeach
                            </td>
                            <td>
                                {{ App\Models\Link::IS_ACTIVE_RADIO[$link->is_active] ?? '' }}
                            </td>
                            <td>
                                @can('link_show')
...
@section('scripts')
    ...
    <script>
        $(function () {
            ...
            let table = $('.datatable-linkCategoryLinks:not(.ajaxTable)').DataTable({ buttons: dtButtons })
