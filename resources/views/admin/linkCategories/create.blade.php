...
@section('content')
    ...
    <div class="card">
        <div class="card-header">
            {{ trans('global.create') }} {{ trans('cruds.linkCategory.title_singular') }}
        </div>

        <div class="card-body">
            <form method="POST" action="{{ route("admin.link-categories.store") }}" enctype="multipart/form-data">
                @csrf
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('page_name') ? 'is-invalid' : '' }}" name="page_name" id="page_name">
                        ...
                        @foreach(App\Models\LinkCategory::PAGE_NAME_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('page_name', 'home') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('image_position') ? 'is-invalid' : '' }}" name="image_position" id="image_position">
                        ...
                        @foreach(App\Models\LinkCategory::IMAGE_POSITION_SELECT as $key => $label)
                            <option value="{{ $key }}" {{ old('image_position', '') === (string) $key ? 'selected' : '' }}>{{ $label }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('link') ? 'is-invalid' : '' }}" name="link[]" id="link" multiple required>
                        @foreach($links as $id => $link)
                            <option value="{{ $id }}" {{ in_array($id, old('link', [])) ? 'selected' : '' }}>{{ $link }}</option>
                        @endforeach
                    </select>
                    @if($errors->has('link'))
                ...
                <div class="form-group">
                    ...
                    @foreach(App\Models\LinkCategory::IS_UTILITIES_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('is_utilities') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="is_utilities_{{ $key }}" name="is_utilities" value="{{ $key }}" {{ old('is_utilities', '0') === (string) $key ? 'checked' : '' }}>
