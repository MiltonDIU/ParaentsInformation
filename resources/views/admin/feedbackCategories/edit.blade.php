        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.feedback-categories.update", [$feedbackCategory->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('title') ? 'is-invalid' : '' }}" type="text" name="title" id="title" value="{{ old('title', $feedbackCategory->title) }}" required>
                    @if($errors->has('title'))
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $feedbackCategory->slug) }}" required>
                    @if($errors->has('slug'))
                        <div class="invalid-feedback">
                            {{ $errors->first('slug') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    @foreach(App\Models\FeedbackCategory::IS_ACTIVE_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('is_active') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="is_active_{{ $key }}" name="is_active" value="{{ $key }}" {{ old('is_active', $feedbackCategory->is_active) === (string) $key ? 'checked' : '' }} required>
