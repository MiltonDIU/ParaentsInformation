                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('link_categories') ? 'is-invalid' : '' }}" name="link_categories[]" id="link_categories" multiple required>
                        @foreach($link_categories as $id => $link_category)
                            <option value="{{ $id }}" {{ (in_array($id, old('link_categories', [])) || $link->link_categories->contains($id)) ? 'selected' : '' }}>{{ $link_category }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    @foreach(App\Models\Link::IS_ACTIVE_RADIO as $key => $label)
                        <div class="form-check {{ $errors->has('is_active') ? 'is-invalid' : '' }}">
                            <input class="form-check-input" type="radio" id="is_active_{{ $key }}" name="is_active" value="{{ $key }}" {{ old('is_active', $link->is_active) === (string) $key ? 'checked' : '' }} required>
