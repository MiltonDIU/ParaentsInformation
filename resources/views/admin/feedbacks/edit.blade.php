        ...
        <div class="card-body">
            <form method="POST" action="{{ route("admin.feedbacks.update", [$feedback->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('feedback_category') ? 'is-invalid' : '' }}" name="feedback_category_id" id="feedback_category_id" required>
                        @foreach($feedback_categories as $id => $entry)
                            <option value="{{ $id }}" {{ (old('feedback_category_id') ? old('feedback_category_id') : $feedback->feedback_category->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                        @endforeach
                    </select>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('content') ? 'is-invalid' : '' }}" type="text" name="content" id="content" value="{{ old('content', $feedback->content) }}" required>
